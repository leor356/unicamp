...
void encontraDist(int **V, int N, int size, int *distancia, int *temp){
	...
	if(size < *distancia){
		for (i=0;i<N;i++){
			...
			encontraDist(Vaux, Naux, size + 1, distancia, temp);
...
int main(){
	...
	encontraDist(V, N, 0, &distancia, temp);
